...
div[data-testid="stVerticalBlock"]:has(> .rv2-card-anchor) {
    background: #FFFFFF;
...
div[data-testid="stVerticalBlock"]:has(> .rv2-badge-anchor) p {
    margin: 0;
    font-size: 12px;
}
div[data-testid="stVerticalBlock"]:has(> .rv2-side-anchor) {
    background: #FFFFFF;
    border: 1px solid #E5E7EB;
    border-radius: 12px;
    padding: 12px 14px;
...
def _card_container():
    with st.container():
        st.markdown("<div class='rv2-card-anchor'></div>", unsafe_allow_html=True)
        yield
...
def _side_container():
    with st.container():
        st.markdown("<div class='rv2-side-anchor'></div>", unsafe_allow_html=True)
        yield
...
def _normalize_courses(courses: Any) -> Dict[str, List[str]]:
    ...
    return {"quick": quick, "upgrade": upgrade, "avoid": avoid}
...
def render_results_page_v2(
    ...
    col_main, col_side = st.columns([0.72, 0.28], gap="large")

    with col_main:
        st.markdown("<div class='rv2-section'>", unsafe_allow_html=True)
        st.markdown("<div class='rv2-title'>\u0646\u062a\u0627\u06cc\u062c \u062a\u062d\u0644\u06cc\u0644 \u0645\u0633\u06cc\u0631 \u0634\u063a\u0644\u06cc</div>", unsafe_allow_html=True)
        st.markdown("<div class='rv2-subtle'>\u062e\u0644\u0627\u0635\u0647\u200c\u0627\u06cc \u06a9\u0648\u062a\u0627\u0647 \u0648 \u0642\u0627\u0628\u0644 \u0627\u0642\u062f\u0627\u0645 \u0627\u0632 \u0645\u0633\u06cc\u0631 \u0634\u0645\u0627</div>", unsafe_allow_html=True)
        ...
        with col_a:
            with _card_container():
                st.metric("\u0627\u062d\u062a\u0645\u0627\u0644 \u0634\u063a\u0644 \u0647\u062f\u0641", probability_label)
        with col_b:
            with _card_container():
                st.metric("\u06af\u067e\u200c\u0647\u0627\u06cc \u0628\u0627\u0632", len(unsolved))
        with col_c:
            with _card_container():
                st.metric("\u062f\u0648\u0631\u0647\u200c\u0647\u0627\u06cc \u0634\u0631\u0648\u0639 \u0633\u0631\u06cc\u0639", len(course_groups.get("quick", [])))
        ...
        with _card_container():
            ...
            if top_gap:
                ...
                why = _truncate(top_gap.get("why_important_fa", ""), 140)
                if why:
                    st.caption(why)
                ...
                st.button("\u0634\u0631\u0648\u0639 \u062a\u0645\u0631\u06cc\u0646 \u06f1\u06f0 \u062f\u0642\u06cc\u0642\u0647\u200c\u0627\u06cc")
        ...
        st.markdown("<div class='rv2-section'>", unsafe_allow_html=True)
        st.subheader("\u06f3 \u06af\u067e \u0627\u0648\u0644\u0648\u06cc\u062a\u200c\u062f\u0627\u0631 \u0634\u0645\u0627")
        if unsolved:
            for gap in unsolved[:3]:
                with _card_container():
                    st.write(gap.get("title_fa", gap.get("gap_id", "")))
                    why = _truncate(gap.get("why_important_fa", ""), 140)
                    if why:
                        st.caption(why)
            ...
            with st.expander("\u0646\u0645\u0627\u06cc\u0634 \u0647\u0645\u0647 \u06af\u067e\u200c\u0647\u0627"):
                for gap in unsolved:
                    st.write(gap.get("title_fa", gap.get("gap_id", "")))
        else:
        ...
        with st.expander("\ud83d\udeab \u062f\u0648\u0631\u0647\u200c\u0647\u0627\u06cc\u06cc \u06a9\u0647 \u0641\u0639\u0644\u0627 \u062a\u0648\u0635\u06cc\u0647 \u0646\u0645\u06cc\u200c\u0634\u0648\u062f"):
        ...
        with st.expander("\u0634\u063a\u0644\u200c\u0647\u0627\u06cc \u0645\u0631\u062a\u0628\u0637"):
        ...
        with st.expander("\u0645\u0634\u0627\u063a\u0644 \u0633\u0637\u062d \u0628\u0639\u062f\u06cc"):
    ...
    with col_side:
        with _side_container():
            st.write("\u0645\u0631\u0627\u062d\u0644")
            steps = [
                ("\u062b\u0628\u062a\u200c\u0646\u0627\u0645", bool(st.session_state.get("auth_completed"))),
                ("\u067e\u0631\u0648\u0641\u0627\u06cc\u0644", bool(st.session_state.get("profile_completed"))),
                ("\u0645\u0635\u0627\u062d\u0628\u0647", bool(st.session_state.get("interview_completed"))),
                ("\u0646\u062a\u0627\u06cc\u062c", True),
            ]
            for label, done in steps:
                status = "\u062a\u06a9\u0645\u06cc\u0644 \u0634\u062f" if done else "\u062f\u0631 \u062d\u0627\u0644 \u0627\u0646\u062c\u0627\u0645"
